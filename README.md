...
A professional, configurable logging utility for modern JavaScript development.  
Works seamlessly in **Browsers**, **Node.js**, and **Serverless** environments.

---

## 💡 The Origin Story

This package was born from the challenges of developing on **Wix Velo**.

In complex serverless ecosystems, logs often get buried under platform warnings, and debugging turns into a nightmare.

**Dev Logger Pro** solves this by giving developers:
...
- **Smart Detection**  
  Automatically formats logs for:

  - Browser → CSS colors
  - Node.js → Clean text output

- **Global Control**  
  Enable/disable all logs with a single `debug` flag.  
  No more commenting out logs.

- **Function Tracing**  
  Instantly see which function triggered the log.
